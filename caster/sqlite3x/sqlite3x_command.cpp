...
sqlite3_command::sqlite3_command(sqlite3_connection &con, const char *sql, ...) : con(con),refs(0) {
	...
	char buffer[1000];
	va_start(list, sql);
	vsprintf(buffer, sql, list);

	const char *tail=NULL;
	if(sqlite3_prepare(con.db, buffer, -1, &this->stmt, &tail)!=SQLITE_OK)
		throw database_error(con);

	this->argc=sqlite3_column_count(this->stmt);
}

sqlite3_command::sqlite3_command(sqlite3_connection &con, const char *sql, va_list list) : con(con),refs(0) {
	char buffer[1000];
	vsprintf(buffer, sql, list);
...
sqlite3_command::sqlite3_command(sqlite3_connection &con, const wchar_t *sql, ...) : con(con),refs(0) {
	...
	wchar_t buffer[1000];
	va_start(list, sql);
	vswprintf(buffer, sizeof(buffer)-1, sql, list);

	const wchar_t *tail=NULL;
	if(sqlite3_prepare16(con.db, buffer, -1, &this->stmt, (const void**)&tail)!=SQLITE_OK)
		throw database_error(con);

	this->argc=sqlite3_column_count(this->stmt);
}

sqlite3_command::sqlite3_command(sqlite3_connection &con, const wchar_t *sql, va_list list) : con(con),refs(0) {
	wchar_t buffer[1000];
	vswprintf(buffer, sizeof(buffer)-1, sql, list);
