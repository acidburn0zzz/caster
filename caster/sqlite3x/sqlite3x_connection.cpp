...
void sqlite3_connection::setChunkSize(unsigned chunkSize) {
#if 0
	if(sqlite3_file_control(db, "main", SQLITE_FCNTL_CHUNK_SIZE,  
		(void*)&chunkSize)!=SQLITE_OK)
		throw database_error("unable to setup chunk size");
#endif
}
...
void sqlite3_connection::create(const char *db) {
	if(sqlite3_open_v2(db, &this->db, SQLITE_OPEN_CREATE, NULL)!=SQLITE_OK)
		throw database_error("unable to create database");
}

void sqlite3_connection::replace(const char *db) {
	if(sqlite3_open_v2(db, &this->db, SQLITE_OPEN_READWRITE|SQLITE_OPEN_CREATE, NULL)!=SQLITE_OK)
		throw database_error("unable to create database");
}
